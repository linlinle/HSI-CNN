function net = KSC_cnn_init(varargin)
% CNN_MNIST_LENET Initialize a CNN similar for MNIST
opts.batchNormalization = true ;%默认打开batchNormalization
...
net.layers{end+1} = struct('type', 'conv', ...%神经元数目500，因为上层50个神经元，所以本层没个神经元突触50个
                           'weights', {{f*randn(2,2,50,500, 'single'),  zeros(1,500,'single')}}, ...%偏置数目500个
                           'stride', 1, ...
                           'pad', 0) ;
...
net.meta.inputSize = [10 10 1] ;%输入数据尺寸：28*28的单通道
net.meta.trainOpts.learningRate = 0.01;%[0.001*ones(1,50) 0.0005*ones(1,30) 0.00001*ones(1,20)] ;%学习率
net.meta.trainOpts.numEpochs = 100;%训练回合次数
net.meta.trainOpts.batchSize = 100 ;%一个批次的传递样本数量
